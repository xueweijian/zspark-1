...
impl CompletedOutputTracker {
    ...
    fn record_emitted_item(&mut self, item: &ResponseItem) {
        if let Some(id) = response_item_id(item) {
            self.emitted_item_ids.insert(id.to_string());
        } else {
            self.emitted_items_without_ids.push(item.clone());
        }
    }

    pub(crate) fn output_items_before_completed(
        &mut self,
        ...
        for value in output_values {
            ...
            let item = self.take_pending_match(&item).unwrap_or(item);
            self.record_emitted_item(&item);
            emit.push(item);
        }
    ...
    fn has_emitted(&self, item: &ResponseItem) -> bool {
        if let Some(id) = response_item_id(item) {
            self.emitted_item_ids.contains(id)
        } else {
            self.emitted_items_without_ids
                .iter()
                .any(|seen| seen == item)
        }
    }

    fn take_pending_match(&mut self, item: &ResponseItem) -> Option<ResponseItem> {
        if let Some(id) = response_item_id(item)
            && let Some(index) = self
                .pending_output_items
                .iter()
                .position(|pending| response_item_id(pending) == Some(id))
        {
            return Some(self.pending_output_items.remove(index));
        }
...
fn parse_completed_output_item(mut value: Value) -> Option<ResponseItem> {
    ...
    match serde_json::from_value::<ResponseItem>(value) {
        Ok(item) => Some(item),
        Err(err) => {
            debug!("failed to parse ResponseItem from response.completed output: {err}");
            None
        }
    }
}

fn should_delay_until_completed(item: &ResponseItem) -> bool {
    matches!(
        item,
...
pub async fn process_sse(
    ...
    loop {
        ...
        let completed_output_items = completed_output_tracker.output_items_before_completed(&event);
...
mod tests {
    ...
    #[tokio::test]
    async fn process_sse_orders_completed_reasoning_before_streamed_function_call() {
