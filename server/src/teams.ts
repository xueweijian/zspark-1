...
interface TeamsEnv { TEAMS_BOT_APP_ID?: string; TEAMS_BOT_APP_SECRET?: string }
...
export async function registerTeamsRoutes(app: FastifyInstance, env: TeamsEnv) {
  app.post('/teams/notify', async (req, reply) => {
    ...
    if (!webhookUrl || !text) return reply.code(400).send({ error: 'webhookUrl and text required' })
  ...
  app.post('/teams/messages', async (req) => {
    ...
    return { ok: true, echoed: req.body }
  })
