 ...
 *
 * Credentials are provided through environment variables set by zspark's main
 * process from Electron safeStorage:
 *
...
interface GmailMcpServerOptions {
  env?: NodeJS.ProcessEnv
  fetchFn?: typeof fetch
  writeMessage?: (msg: JsonRpcResponse) => void
}

export function createGmailMcpServer(options: GmailMcpServerOptions = {}) {
  const env = options.env ?? process.env
  const fetchFn = options.fetchFn ?? fetch
  const writeMessage = options.writeMessage ?? ((msg: JsonRpcResponse) => {
  ...
  async function callMailSend(params: { to: string[]; cc?: string[]; subject: string; body: string }) {
    ...
    const rfc822 = buildRfc822Message({
      ...
      body: params.body ?? '',
      from: env.ZSPARK_GMAIL_USER_EMAIL || undefined
    })
...
export function start() {
  ...
  rl.on('line', (line) => {
    ...
    server.handleRequest(req).catch((err) => {
      process.stdout.write(`${JSON.stringify({ jsonrpc: '2.0', id: req.id ?? null, error: { code: -32000, message: err?.message ?? String(err) } })}\n`)
    })
  })
